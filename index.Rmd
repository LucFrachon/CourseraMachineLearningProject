...
fullSet <- data.frame(select(rawData, (X : raw_timestamp_part_2), new_window,
                      num_window), colSelect,
...
plot(fullSet$X, fullSet$num_window, pch = as.character(fullSet$classe), col = fullSet$user_name, xlab = "Row index", ylab = "num_window")
```
...
```{r echo = FALSE}
NAsByCol <- sapply(names(fullSet), function(c){sum(is.na(fullSet[, c])) / length(fullSet[, c])})
```
...
```{r}
trainIndex <- createDataPartition(fullSetNoNAs$classe, p = 0.6, list = F)
...
forestFit <- train(classe ~ ., method = "rf", data = train1[, 7:59], prox = T, trControl = fitControl)
...
accuraciesWithPCA <- function(t, outcome, model, trainData, cvData,
    ...
    results[1] <- sum(cvPredPCA == cvData[[outcome]]) / length(cvPredPCA)
